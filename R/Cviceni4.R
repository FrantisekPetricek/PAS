...
hist(bmi, main = "BMI", border = "black", col = "green",  ylab = "Hustota", xlab = "BMI", freq = F)

jadro <- density(bmi, bw = 3)
...
syst <- Stulong$syst1
hist(syst)
meze <- hist(syst, plot = F)$breaks
pocty <- hist(syst, plot = F)$counts
m <- which.max(pocty)
A <- meze[m]
h <- meze[2] - meze[1]
d0 <- pocty[m] - pocty[m-1]
...
exp(DM)
#nevihoda - jen pro kladné hodnoty
...
cukr_t <- sqrt((cukr - min(cukr)) / (max(cukr) - min(cukr)))

hist(cukr_t)
boxplot(cukr_t)
HM <- mean(cukr_t) + 3 * sd(cukr_t)
cukr_t[cukr_t > HM]
HM <- quantile(cukr_t, 0.75) + 1.5 * IQR(cukr_t)
