...
rm <-Boston$rm
#symetrické, jednovrcholové, zadne odlehle hodnoty, spicate
hist(rm, col="red", border = "black", xlab = "rm", main = "Histogram")
...
tax <- Boston.kat$tax.k
chas <- Boston.kat$chas
t <- table(tax,chas)

round(addmargins(prop.table(t)), 3)
round(addmargins(prop.table(t,1)), 3)
round(addmargins(prop.table(t,2)), 3)
...
# a co korelace
# -1 nepřímá zavislost
# 0 nekorelovanost (= linearni zavislost)
# 1 přímá závislost
...
#cov(rm, medv, use = "complete.obs")
cor(medv, rm, use = "complete.obs")
